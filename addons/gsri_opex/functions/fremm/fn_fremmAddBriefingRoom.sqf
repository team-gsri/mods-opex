...
if(isServer) then {
    ...
    // Briefing screen, projector and remote
    private _screen = "SCMS_Large_screen" createVehicle getPos _briefing;
    _screen attachTo [_briefing, [4.8,1,0.2]];
    _screen setDir 180;
    _ship setVariable ["GSRI_FREMM_briefingRoom_screen", _screen, true];
    private _projector = "Land_Projector_01_F" createVehicle getPos _briefing;
    _projector attachTo [_briefing, [2,1,1.2]];
    [_projector, [270,15,180]] call BIS_fnc_setObjectRotation;
    private _remote = "Land_Tablet_02_black_F" createVehicle getPos _briefing;
    _remote attachTo [_briefing, [3,3,0.2]];
    [_remote, [180,-90,0]] call BIS_fnc_setObjectRotation;
    _ship setVariable ["GSRI_FREMM_briefingRoom_remote", _remote, true];
...
if!(isDedicated) then {
    [
        [_ship getVariable "GSRI_FREMM_briefingRoom_screen"],
        [_ship getVariable "GSRI_FREMM_briefingRoom_remote"],
        ["destroyer_habs\data\ecrans\ecrans_debrief3_co.paa", "destroyer_habs\data\ecrans\ecrans_debrief1_co.paa", "destroyer_habs\data\ecrans\ecrans_debrief2_co.paa"],
        ["Logo GSRI", "Debriefing", "Ambiance"],
        0,
        "Debriefing"
